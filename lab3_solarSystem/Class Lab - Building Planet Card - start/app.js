...
const Planet =(props)=>{
  return(
      <div class="card">
        ...
        <button onClick={()=>{props.removePlanet(props.id)}}> Remove Planet</button>
      </div>
    );
}
...
class App extends React.Component {
  ...
  deletePlanet=(id)=>{
    this.setState(prevState=>{
      return{
        planets:prevState.planets.filter(singlePlanet => singlePlanet.id !==id)
      };
    });
  }
  ...
  render(){
    return(
      <div class="container">
        {this.state.planets.map( singlePlanet=>{
          return <Planet 
            id={singlePlanet.id}
            name={singlePlanet.name}
            diameter={singlePlanet.diameter}
            moons={singlePlanet.moons}
            text={singlePlanet.desc}          
            url={singlePlanet.url}
            alt={"image of "+singlePlanet.name}
            removePlanet={this.deletePlanet}
          />
        })}
      </div>
    );
